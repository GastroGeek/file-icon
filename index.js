...
const validate = (file, options) => {
  ...
  if (options.binaryPathModifier && typeof options.binaryPathModifier !== 'function') {
    throw new Error('binaryPathModifier must be a function')
  }
...
export async function fileIconToBuffer (file, options) {
  ...
  const mapper = async file => {
    const { stdout } = await execFileP(options.binaryPathModifier(binary), [toCLIArgument(file, options)], spawnOptions)
    return stdout
  }
...
export async function fileIconToFile (file, options) {
  ...
  await execFileP(options.binaryPathModifier(binary), [toCLIArgument(file, options)], spawnOptions)
}
